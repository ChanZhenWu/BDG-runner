...
#创建一个新的Excel文件
#my $log_report = Spreadsheet::WriteExcel->new('CPK_report.xls');
my $log_report = Excel::Writer::XLSX->new('CPK_report.xlsx');
...
$log_counter++;
	...
	open NLog,">head";
		...
		if ($line =~ "\@BTEST")
       	...
       	{
       		$col = 0;
       		#2.2# $headN = substr ($line, 8, length($line)-3-8),"\n";  #生成主测试名
       		$headN = $string[1];
       		#print "\n".$headN;
        ...
        #2.2# elsif ($line =~ "\@LIM2" and length($line) == 59)
        elsif ($line =~ "\@LIM2" and scalar @string == 5)
       	{
       		#print $headN, "\r";
       		print NLog $headN, "\r";
       		$workbook-> write($row, $col, $headN, $format_item);					#输出测试名，单项测试
			$workbook-> write($row, 1, substr($line,4,3), $format_data);			#输出TYPE
       		#2.2# $workbook-> write($row, 3, substr($line,30,13), $format_data);	#输出上限值
       		$workbook-> write($row, 3, $string[3], $format_data);
       		#2.2# $workbook-> write($row, 4, substr($line,44,13), $format_data);	#输出下限值
        ...
        #2.2# elsif ($line =~ "\@LIM3" and length($line) == 73)
        elsif ($line =~ "\@LIM3" and scalar @string == 6)
       	{
       		#print $headN, "\r";
       		print NLog $headN, "\r";
       		$workbook-> write($row, $col, $headN, $format_item);					#输出测试名，单项测试
			$workbook-> write($row, 1, substr($line,4,3), $format_data);			#输出TYPE
       		...
       		#2.2# $workbook-> write($row, 3, substr($line,44,13), $format_data);	#输出上限值
       		$workbook-> write($row, 3, $string[4], $format_data);
       		#2.2# $workbook-> write($row, 4, substr($line,58,13), $format_data);	#输出下限值
       	...
       	elsif ($line =~ "\@A-")
       	{
			...
			$workbook-> write($row, 1, substr($line,4,3), $format_data);  			#输出TYPE
       		#2.2# $workbook-> write($row, 3, substr($line,index($line,"\@LIM")+6,13), $format_data);	#输出上限值
       		$workbook-> write($row, 3, $string[4], $format_data);
       		#2.2# $workbook-> write($row, 4, substr($line,rindex($line,"|")+1,13), $format_data);  		#输出下限值
       		$workbook-> write($row, 4, substr($string[5],0,13), $format_data);
			$workbook-> write_formula($row, 5, "=MAX(L".($row+1).":AAA".($row+1).")", $format_data);  #输出Max
			$workbook-> write_formula($row, 6, "=MIN(L".($row+1).":AAA".($row+1).")", $format_data);  #输出Min
			$workbook-> write_formula($row, 7, "=AVERAGE(L".($row+1).":AAA".($row+1).")", $format_data);  #输出Average
